...
from differential_geometry.bundles_new import embed_fibre_pairs
...
# ==========================================================
# === Tangent and cotangent bundles drawn together
# ==========================================================

def plot_fibre_pair_sheet(
    ax,
    tangent_bundle,
    cotangent_bundle,
    param_point,
    ...
    """
    Plot the sheet  T_pM x T*_pM  over one base point: the planar patch spanned by the
    tangent fibre (as drawn by the TangentBundle) and the cotangent fibre (as drawn by
    the CotangentBundle). With the default directions and a planar ring, it is the
    vertical half-plane attached along the tangent fibre and extending radially outward.
    ...
    Args:
        ...
        param_point: Array (base_dim,) — the base point in parameter space.
        tangent_range: Tuple (u_min, u_max) of tangent fibre components to draw.
        cotangent_range: Tuple (w_min, w_max) of cotangent fibre components to draw.
        resolution: Tuple (n_tangent, n_cotangent) of grid points of the patch.
        color, alpha: Fill of the sheet.
        edgecolor, edge_linewidth: Outline of the sheet.
        zorder: Drawing order.
        label: Optional LaTeX label, placed at the outer top corner of the sheet.
        label_fontsize: Font size of the label.
        label_offset: Tuple (dx, dy, dz) offset of the label in ambient space.
    """
    tangent_values = jnp.linspace(tangent_range[0], tangent_range[1], resolution[0])
    cotangent_values = jnp.linspace(cotangent_range[0], cotangent_range[1], resolution[1])
    tangent_grid, cotangent_grid = jnp.meshgrid(tangent_values, cotangent_values)

    n_points = tangent_grid.size
    param_points = jnp.broadcast_to(jnp.asarray(param_point).reshape(1, -1), (n_points, 1))

    embedded = np.asarray(embed_fibre_pairs(
        tangent_bundle, cotangent_bundle, param_points,
        tangent_grid.reshape(-1, 1), cotangent_grid.reshape(-1, 1),
    ))

    X = embedded[:, 0].reshape(tangent_grid.shape)
    Y = embedded[:, 1].reshape(tangent_grid.shape)
    Z = embedded[:, 2].reshape(tangent_grid.shape)

    ax.plot_surface(X, Y, Z, color=color, alpha=alpha, edgecolor="none",
                    shade=False, zorder=zorder)

    corners = np.array([
        [tangent_range[0], cotangent_range[0]], [tangent_range[0], cotangent_range[1]],
        [tangent_range[1], cotangent_range[1]], [tangent_range[1], cotangent_range[0]],
        [tangent_range[0], cotangent_range[0]],
    ])
    outline = np.asarray(embed_fibre_pairs(
        tangent_bundle, cotangent_bundle,
        jnp.broadcast_to(jnp.asarray(param_point).reshape(1, -1), (5, 1)),
        corners[:, :1], corners[:, 1:],
    ))
    ax.plot(outline[:, 0], outline[:, 1], outline[:, 2], color=edgecolor,
            linewidth=edge_linewidth, zorder=zorder + 1)

    if label is not None:
        position = outline[2] + np.asarray(label_offset)
        ax.text(position[0], position[1], position[2], label,
                fontsize=label_fontsize, color=edgecolor)


def plot_covector_in_fibre(
    ax,
    cotangent_bundle,
    param_point,
    covector_component,
    ...
    """
    Plot one covector inside its fibre, as an arrow along the direction in which the
    cotangent fibre is drawn.

    By default the arrow starts at the base point (a point of the zero section of T*M).
    If a TangentBundle and a vector component are given, the arrow starts instead at the
    tip of that vector, i.e. the pair (v, X_p) in T_pM x T*_pM is drawn as a single
    point of the sheet plotted by plot_fibre_pair_sheet.

    Args:
        ax: Matplotlib 3D axis.
        cotangent_bundle: CotangentBundle object.
        param_point: Array (base_dim,) — the base point in parameter space.
        covector_component: Component of the covector w.r.t. d(theta).
        tangent_bundle: Optional TangentBundle, to attach the arrow to a vector.
        vector_component: Component of that vector w.r.t. d/d(theta).
        color, linewidth, mutation_scale, arrowstyle, alpha: Arrow styling.
        label: Optional LaTeX label (e.g. r"$\\mathsf{X}_p$").
        label_fontsize: Font size of the label.
        label_offset: Tuple (dx, dy, dz) offset of the label in ambient space.

    Returns:
        Tuple (start, end) with the ambient coordinates of both ends of the arrow.
    """
    param_points = jnp.asarray(param_point).reshape(1, -1)

    if tangent_bundle is None:
        start = np.asarray(cotangent_bundle.embed(
            cotangent_bundle.bundle_points(param_points, jnp.array([[0.0]]))))[0]
        end = np.asarray(cotangent_bundle.embed(
            cotangent_bundle.bundle_points(param_points, jnp.array([[covector_component]]))))[0]
    else:
        start = np.asarray(embed_fibre_pairs(
            tangent_bundle, cotangent_bundle, param_points,
            jnp.array([[vector_component]]), jnp.array([[0.0]])))[0]
        end = np.asarray(embed_fibre_pairs(
            tangent_bundle, cotangent_bundle, param_points,
            jnp.array([[vector_component]]), jnp.array([[covector_component]])))[0]
...
def plot_paired_section(
    ...
    """
    Plot the pair of fields p -> (X_p, Theta_p) as a single curve: its displacement
    along the tangent fibres is the vector field and its displacement along the
    cotangent fibres is the covector field. Forgetting the cotangent part gives back the
    section of TM drawn by plot_section_on_total_space; forgetting the tangent part gives
    the section of T*M, which can be drawn on its own with plot_section_on_total_space
    and the CotangentBundle.

    Args:
        ax: Matplotlib 3D axis.
        tangent_bundle: TangentBundle object.
        cotangent_bundle: CotangentBundle object over the same manifold.
        vector_field: VectorField on the base manifold.
        covector_field: CovectorField on the base manifold.
        param_range: Tuple (t_min, t_max) of the base parameter.
        n_points: Number of sampling points.
        color, linewidth, alpha, zorder: Line styling.
        label: Optional LaTeX label (e.g. r"$\\Theta$").
        label_fontsize: Font size of the label.
        label_position: Position of the label along the curve, in [0, 1].
        label_offset: Tuple (dx, dy, dz) offset of the label in ambient space.

    Returns:
        Array (n_points, ambient_dim) with the plotted curve.
    """
    param_points = jnp.linspace(param_range[0], param_range[1], n_points).reshape(-1, 1)
    vector_components = jnp.atleast_2d(vector_field.evaluate_in_param_space(param_points))
    covector_components = jnp.atleast_2d(covector_field.evaluate_in_param_space(param_points))

    embedded = np.asarray(embed_fibre_pairs(
        tangent_bundle, cotangent_bundle, param_points, vector_components, covector_components))
